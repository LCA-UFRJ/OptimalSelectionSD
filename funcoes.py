...
def ComponenteFortementeConexo(dictAdjacencias={}, estado1="", estado2="", estados=[]):
    ...
    # print(caminho1)
    # print(caminho2)
    return [isCFC, caminho1, caminho2, caminho]
                ...
                for ee in automato.EeE:
                    if ee[0] == elemento and ee[2] == elemento and ee[1] not in eventosRenomeados:
                        ...
                        resposta.append(
                            ...
                            ee[1] + ") entre eles")
                        if ee[1] not in ciclo:
                            ciclo.append(ee[1])
                        FcaminhoEventos1.append([ee[1]])
                        FcaminhoEventos2.append([])
                        Fcaminho1.append(elemento)
                        Fcaminho2.append([])
    isCFC = False
    isEvento = False
    eventosProblema = ""
            ...
            if estado1 != estado2:
                ...
                if isCFC == False and "Y" in estado1 and "Y" in estado2:
                    ...
                    while contador < len(caminho1) - 1:
                        for ee in automato.EeE:
                            ...
                            if ee[0] == caminho1[contador] and ee[2] == caminho1[contador + 1]:
                                # if ee[0] == caminho1[contador] and ee[2] == caminho1[contador + 1] and ee[1] not in caminhoEventos1:
                    ...
                    for ev in caminhoEventos1:
                        if ev not in eventosRenomeados:
                            ...
                            if eventosProblema == "":
                                ...
                                eventosProblema += ", " + ev
                    if caminhoEventos1 != []:
                        FcaminhoEventos1.append(caminhoEventos1)
                        Fcaminho1.append(estado1)
                    caminho2 = ret[2][::-1]
                    caminhoEventos2 = []
                    contador = 0
                    # print((estado1))
                    # print((caminho1))
                    while contador < len(caminho2) - 1:
                        for ee in automato.EeE:
                            # print(ee[0])
                            # print( caminho1[contador] )
                            # print(ee[2])
                            # print( caminho1[contador+1] )
                            if ee[0] == caminho2[contador] and ee[2] == caminho2[contador + 1]:
                                # if ee[0] == caminho1[contador] and ee[2] == caminho1[contador + 1] and ee[1] not in caminhoEventos1:
                                caminhoEventos2.append(ee[1])
                        contador += 1
                    for ev in caminhoEventos2:
                        if ev not in eventosRenomeados:
                            isEvento = True
                            if eventosProblema == "":
                                eventosProblema = ev
                                ...
                                eventosProblema += ", " + ev
                    if caminhoEventos2 != []:
                        FcaminhoEventos2.append(caminhoEventos2)  # print((caminho2))
                        Fcaminho2.append(estado2)
                    ...
                    if isCFC and isEvento:
                        ...
                        if eventosProblema not in ciclo:
                            ciclo.append(eventosProblema)
    if isCFC == False and isAutolaco == False:
        resposta.append("Linguagem gerada é diagnosticável")
    if resposta == ["Linguagem gerada é diagnosticável"]:
        resumo = ["Linguagem gerada é diagnosticável"]
    else:
        resumo = ["Linguagem gerada não é diagnosticável"]

    return ([resposta, ciclo, resumo, [FcaminhoEventos1, FcaminhoEventos2, Fcaminho1, Fcaminho2]])
...
def automatoGn(aut, EF):
    ...
    for SF in EF:
        for ev in aut.eventos:
            if ev != SF:  # and ev not in eventosN:
                eventosN.append(ev)
    for ev in aut.eventos:
        if ev not in EF:
            EeEN.append(["N", ev, "N"])

    An = Automato(estadosN, aut.eventos, ["N"], [], EeEN)
    # PreGn = ComposicaoParalela(aut, An)
...
def automatoGni(aut, Gn, EF):
    ...
    comp = filtraElementosAutomato(aut.estados, eventosN, Gn.estadoInicial, aut.estadosMarcados, Gn.EeE,
                                   dictSimilares)
    # [estadosNovo, eventosnovo, estadoInicialNovo, estadoMarcadoNovo, EeENovo]
    # FALTA MARCAR ESTADOS
    # print(listaSimilares)
    # PreGni = Automato(comp[0], comp[1], comp[2], comp[3], comp[4], {})
    # Gni = ParteAcessivel(PreGni)
    # print(aut.eventos)
    # print(eventosN)
    Gni = Automato(comp[0], eventosN, comp[2], comp[3], comp[4], {})
    # print(Gni.EeE)
    return (Gni)
...
def limpaGni(aut):
    ...
    for est1 in estadosLinha:
        for est2 in estadosLinha:
            ...
            if True not in temEventoSobrando1 and True not in temEventoSobrando2 and temEventoSobrando1 != [] and temEventoSobrando2 != []:
            ...
            for ee1 in EeELinha:
                for ee2 in EeELinha:
                    if est1 == ee1[0] and est2 == ee2[0] and ee1[1] == ee2[1] and ee1[2] == ee2[2]:
                        dictTalvez = ComparaSimilares(est1, est2, dictTalvez)
    ...
    for est1 in estadosLinha:
        for est2 in estadosLinha:
            ...
            for ee1 in EeELinha:
                for ee2 in EeELinha:
                    if ee1[0] == est1 and ee2[0] == est2 and ee1[1] == ee2[1]:

                        if ee1[2] == ee2[2]:
                            # if est1 in dictTalvez.keys() and est2 in dictTalvez.keys() and ee1[2] == ee2[2]:
                            ...
                            temEventoSobrando1.append(True)
                            temEventoSobrando2.append(True)
                    elif ee1[0] == est1 and ee2[0] == est2 and ee1[1] != ee2[1]:
                        if ee1[1] not in dictEventosAtivos[est2]:
                            temEventoSobrando1.append(True)
                        if ee2[1] not in dictEventosAtivos[est1]:
                            temEventoSobrando2.append(True)
                        # else:
                        #     temEventoSobrando1.append(True)
                        #     temEventoSobrando2.append(True)
            ...
            if True not in temEventoSobrando1 and True not in temEventoSobrando2 and temEventoSobrando1 != [] and temEventoSobrando2 != []:
                dictJuntarEstadosiguais = ComparaSimilares(est1, est2)
    # print("dictEventosAtivos")
    # print(dictEventosAtivos)
    # print("dictTalvez")
    # print(dictTalvez)
    # print("dictJuntarEstadosiguais")
    # print(dictJuntarEstadosiguais)
...
def DiagnoseModulosOtimizada(diagnose, eventosRenomeados, autr=[], verificadoresUnitarios=[]):
    i = 1
    j = 1
    verificador = []
    cfc = {}
    ...
    listaProblemas = []
    for aut in autr:
        autRenomeados[caut] = aut
        verUni[caut] = verificadoresUnitarios[caut - 1]
        caut = caut + 1
    # print(autRenomeados)
    # print(verUni)
    # analise em dupla
    for ver_i in diagnose.keys():
        for ver_j in diagnose.keys():

            if ver_i[3] < ver_j[3] and len(ver_i) == 4:
                # print(ver_i)
                # print(ver_j)
                intersecao = False
                for problemas_i in diagnose[ver_i]:
                    for problemas_j in diagnose[ver_j]:
                        if problemas_i == problemas_j:
                            intersecao = True
                            chave = ver_i + "|" + ver_j
                            if chave in cfc.keys():
                                cfc[chave].append(problemas_i)
                            else:
                                cfc[chave] = [problemas_i]
                            listaProblemas = cfc[chave]
                            listaProblemas.append(problemas_i)
                            cfc[chave] = listaProblemas
                if intersecao == False:
                    # print("diagnostica com os verificadores ", ver_i, " e ", ver_j)
                    umaBase = ver_i + "|" + ver_j
                    cfc[ver_i + "|" + ver_j] = []
                    if umaBase not in base:
                        base.append(umaBase)
                    # verificador.append(ComposicaoParalela(verUni[int(ver_i[3])], autRenomeados[int(ver_j[3])]))

                # else:

                # print("não diagnostica apenas com os verificadores ", ver_i, " e ", ver_j)
                # base.append(ver_i + "|" + ver_j)
    # analise do resto
    teste = 0
    while teste < len(autr):  # and len(base) ==0:
        # print("RESTO")
        # for k in cfc.keys():
        #  print(k)
        # print(base)
        ...
        for c in cfc.keys():
            chavesCFC.append(c)
            # print(chavesCFC)
        for ver_n in diagnose.keys():
            for e_chave in chavesCFC:
                if ver_n not in e_chave and ver_n[3] < e_chave[3] and (ver_n + "|" + e_chave) not in cfc.keys():
                    intersecao = False
                    for problemas_n in diagnose[ver_n]:
                        if problemas_n in cfc[e_chave]:
                            intersecao = True
                            n_chave = ver_n + "|" + e_chave
                            # cfc[e_chave] = problemas_n
                            if n_chave in cfc.keys():
                                if problemas_n not in cfc[n_chave]:
                                    cfc[n_chave].append(problemas_n)
                            else:
                                cfc[n_chave] = [problemas_n]
                            listaProblemas = cfc[e_chave]
                            if problemas_n not in listaProblemas:
                                listaProblemas.append(problemas_n)
                            cfc[n_chave] = listaProblemas
                    if intersecao == False:
                        # print("diagnostica")
                        # print(ver_n)
                        # print(e_chave)
                        # print(problemas_n)
                        # print(cfc[e_chave])
                        # print("diagnostica com os verificadores ", ver_n, " e ", e_chave.replace("|", "  e  "))
                        umaBase = e_chave + "|" + ver_n
                        cfc[ver_n + "|" + e_chave] = []
                        if umaBase not in base:
                            base.append(umaBase)
                        # verificador.append(ComposicaoParalela(verUni[int(ver_i[3])], autRenomeados[int(ver_j[3])]))
                    # else:
                    # print("não diagnostica com os verificadores ", ver_n, " e ", e_chave.replace("|", " e "))
                    # print("não diagnostica")
                    # print(ver_n)
                    # print(e_chave)
                    # base.append(ver_i + "|" + ver_j)
    ...
    if teste == len(autr):
        ...
        for b in base:
            b1 = b.split("|")
            b1.sort()
            baseF = "|".join(str(x) for x in b1)
            if baseF not in baseFinal:
                baseFinal.append(baseF)

        # print(baseFinal)

    return [verificador, baseFinal]


def SequenciaAteAquiProfundidade(dictAdjacencias={}, estadoAlvo="", estadoInicial="", estados=[]):
    isCFC = False
    caminho = []
    if estadoAlvo != estadoInicial:
        ret = BuscaPorProfundidade(dictAdjacencias, estadoInicial, estados, 0, {}, {}, {})
        alcancaEst2 = ret[1].get(estadoAlvo)
        # print(ret)
        if alcancaEst2:  # alcancaEst1 and alcancaEst2:
            isCFC = True
            proximo = estadoAlvo
            while proximo != []:
                caminho.append(proximo)
                proximo = ret[4].get(proximo)
                # print(proximo)

    return [isCFC, caminho]


def SequenciaAteAquiLargura(dictAdjacencias={}, estadoAlvo="", estadoInicial="", estados=[]):
    isCFC = False
    caminho = []
    if estadoAlvo != estadoInicial:
        ret = BuscaPorLarguraGfLinha(dictAdjacencias, estadoInicial, estados)  # , 0, {}, {}, {})
        alcancaEst2 = ret[1].get(estadoAlvo)
        # print(ret)
        if alcancaEst2:  # alcancaEst1 and alcancaEst2:
            isCFC = True
            ...
            while proximo != []:
                caminho.append(proximo)
                proximo = ret[1].get(proximo)
                # print(proximo)

    return [isCFC, caminho]


def MontaGfLinha(verBase, caminho1, caminho2, estadoCFC1, estadoCFC2):
    # ver = ver3
    # verNovo = ver1
    # print( verBase.estadoInicial[0])
    # print(estadoCFC1[0])
    adj = MontaDictAdjacencias(verBase.estados, verBase.eventos, verBase.EeE)

    #print(adj)
    #print(estadoCFC1[0])
    #print(verBase.estadoInicial[0])
    #print(verBase.estados)
    ret = SequenciaAteAquiLargura(adj, estadoCFC1[0], verBase.estadoInicial[0], verBase.estados)
    ...
    caminhoEventos = []
    contador = 0
    while contador < len(caminho) - 1:
        for ee in verBase.EeE:
            # print(ee[0])
            # print( caminho1[contador] )
            # print(ee[2])
            # print( caminho1[contador+1] )
            if ee[0] == caminho[contador] and ee[2] == caminho[contador + 1]:
                # if ee[0] == caminho1[contador] and ee[2] == caminho1[contador + 1] and ee[1] not in caminhoEventos1:
                caminhoEventos.append(ee[1])
        contador += 1
    # print(caminhoEventos)

    cont = 0
    estIni = str(cont)
    estados = []
    estados.append(estIni)
    eventos = verBase.eventos
    EeE = []
    estAtual = estIni
    for ev in caminhoEventos:
        cont = cont + 1
        estProx = str(cont)
        estados.append(estProx)
        EeE.append([estAtual, ev, estProx])
        estAtual = estProx
    # print(EeE)

    if caminho2[0] == []:
        EeE.append([estAtual, caminho1[0][0], estAtual])
        # print(EeE)
    else:
        estCiclo = estAtual
        for ev in caminho1:#for ev in caminho1[2]:
            c = 1
            if c == len(caminho1):  # if c == len(caminho2[2]):

                cont = cont + 1
                estProx = str(cont)
                estados.append(estProx)
                EeE.append([estAtual, ev[0], estProx])
                estAtual = estProx
            else:
                cont = cont + 1
                estProx = str(cont)
                ...
                estAtual = estProx
            c = c + 1
        # for ev in caminho2:#for ev in caminho2[2]:
        #     c = 1
        #     # print(len(caminho2[2]))
        #     if c == len(caminho2):#if c == len(caminho2[2]):
        #         cont = cont + 1
        #         estProx = str(cont)
        #         # estados.append(estProx)
        #         EeE.append([estAtual, ev, estCiclo])
        #         estAtual = estProx
        #     else:
        #         cont = cont + 1
        #         estProx = str(cont)
    ...
    # print(estados)
    # print(eventos)
    gflinha = Automato(estados, eventos, estIni, [], EeE)

    return gflinha


def TesteGfLinha(gflinha, verNovo, eventosRenomeados):
    teste = ComposicaoParalela(gflinha, verNovo)
    # print(teste.EeE)

    resp = VerificadorDiagnosticabilidade(teste, eventosRenomeados)#['S1R1', 'S2R1', 'S1R2', 'S2R2', 'S1R3', 'S2R3', 'S1R4', 'S2R4'])
    printar = [resp[2], resp[1], teste, resp[3]]
    return printar


def AdicionaModuloEDiagnostica(verVelho, verNovo, eventosRenomeados):
    teste = ComposicaoParalela(verVelho, verNovo)
    # print(teste.EeE)
...
def BuscaOtimizada(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    # minimumSubset = False
    # print("diagnose")
    diagnosticoDict = {}
    diagnose = {}
    for k, v in automatosVerDict.items():
        # print(k)
        resp = VerificadorDiagnosticabilidade(v,
                                              eventosRenomeados)  # ['S1R1', 'S2R1', 'S1R2', 'S2R2', 'S1R3', 'S2R3', 'S1R4', 'S2R4'])
        # printar = [resp[2], resp[1]]
        diagnosticoDict[k] = {"diagnostico": resp[2], "problemas": resp[1], "info": resp[3]}
        # print(diagnosticoDict)
        diagnose[k] = resp[1]
        ultimosVerificadores.append(k)

    ret = DiagnoseModulosOtimizada(diagnose, eventosRenomeados, autR, verUni)

    return [ret, automatosVerDict]


def BuscaGfLinha(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    minimumSubset = False
    # print("diagnose")
    diagnosticoDict = {}
    ...
    # print(ultimosVerificadores)
    penultimosVerificadores = ultimosVerificadores
    unitarios = ultimosVerificadores
    # print("unitarios")
    # print(unitarios)

    limite = 2 ** len(ultimosVerificadores) - 1
    # print(len(ultimosVerificadores))
    # print(limite)
    verif = []
    # while len(verificadores) < 5:#limite:
    novosV = []
    for v in unitarios:
        verif.append(v)
    contGf = 1
    while len(diagnosticoDict) < limite and minimumSubset == False:#len(automatosVerDict) < limite and minimumSubset == False:
        # print(minimumSubset)
        novosV = []
        for v1 in penultimosVerificadores:
            for v2 in unitarios:
                # print(unitarios)
                # print(penultimosVerificadores)
                # print(v1)
                # print(v2)
                aux2 = int(v2[-1])
                aux1 = int(v1[-1])
                newV = v1 + "|" + v2
                if v1 + "|" + v2 not in automatosVerDict.keys() and v2 + "|" + v1 not in automatosVerDict.keys() and v1 not in v2 and v2 not in v1 and len(
                        v2) == 4 and aux2 > aux1:

                    # inserir analise aqui!!!
                    if v1 not in automatosVerDict.keys():
                        gflinha1 = auxGfLinhaDict[v1]
                        # proximas 4 linhas de codigo adicionadas para calcular o verificador quando não foi calculado anteriormente
                        GfLinhaDict["gf'" + v1] = gflinha1
                        resp = AdicionaModuloEDiagnostica(automatosVerDict[v1[0:-5]], automatosVerDict[v1[-4:]], eventosRenomeados)
                        automatosVerDict[v1] = resp[2]
                        # printar = [resp[0], resp[1]]
                        diagnosticoDict[v1] = {"diagnostico": resp[0], "problemas": resp[1], "info": resp[3]}
                        # print("diag")
                        # print(diagnosticoDict[newV]["diagnostico"])
                    # verificador1 = automatosVerDict[v1]
                    # verificador2 = automatosVerDict[v2]
                    # print(diagnosticoDict[v1]["info"])
                    else:
                        verificador1 = automatosVerDict[v1]
                        if v1 not in usaGfLinhaDict.keys():
                            gflinha1 = MontaGfLinha(verificador1, diagnosticoDict[v1]["info"][0],
                                                    diagnosticoDict[v1]["info"][1], diagnosticoDict[v1]["info"][2],
                                                    diagnosticoDict[v1]["info"][3])
                            usaGfLinhaDict[v1] = gflinha1
                        else:
                            gflinha1 = usaGfLinhaDict[v1]
                    #if "gf'" + v1 not in GfLinhaDict.keys():
                    #    print("gf'" + v1)
                    #    for est in gflinha1.estados:
                    #        print(est)
                    #    for ee in gflinha1.EeE:
                    #        print(ee)

                    verificador2 = automatosVerDict[v2]
                    GfLinhaDict["gf'" + v1] = gflinha1

                    ret = TesteGfLinha(gflinha1, verificador2, eventosRenomeados)
                    GfLinhaDict["gf'" + v1 + "|" + v2 + "|" + str(contGf)] = ret[2]
                    # print("printar")
                    # print(printar)
                    if ret[0] == ['Linguagem gerada é diagnosticável']:
                        if v1 not in automatosVerDict.keys():
                            resp = AdicionaModuloEDiagnostica(automatosVerDict[v1[0:-5]], automatosVerDict[v1[-4:]], eventosRenomeados)
                            automatosVerDict[v1] = resp[2]
                            printar = [resp[0], resp[1]]
                            ...
                            # print(diagnosticoDict[newV]["diagnostico"])
                        verificador1 = automatosVerDict[v1]
                        resp = AdicionaModuloEDiagnostica(verificador1, verificador2, eventosRenomeados)
                        automatosVerDict[newV] = resp[2]
                        printar = [resp[0], resp[1]]
                        diagnosticoDict[newV] = {"diagnostico": resp[0], "problemas": resp[1], "info": resp[3]}
                        # print("diag")
                        # print(diagnosticoDict[newV]["diagnostico"])
                        if diagnosticoDict[newV]["diagnostico"] == ['Linguagem gerada é diagnosticável']:
                            minimumSubset = True
                    else:
                        diagnosticoDict[newV] = {"diagnostico": ret[0], "problemas": ret[1], "info": ret[3]}
                        auxGfLinhaDict[newV] = gflinha1
                        auxGfLinhaDict[v2] = gflinha1
                        usaGfLinhaDict[v2] = gflinha1
                        #usaGfLinhaDict[newV] = gflinha1
                    # print(diagnosticoDict)
                    ...
                    # print(verificadores)
                contGf = contGf + 1
        penultimosVerificadores = novosV
        # print(len(automatosVerDict))

    # print(automatosVerDict)
    # print(diagnosticoDict)
    #print(usaGfLinhaDict)
    return [automatosVerDict, diagnosticoDict, GfLinhaDict, auxGfLinhaDict]


def BuscaInteligente(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    minimumSubset = False
    # print("diagnose")
    diagnosticoDict = {}
    ...
    for v in unitarios:
        verif.append(v)
    while len(automatosVerDict) < limite and minimumSubset == False:
        # print(minimumSubset)
        novosV = []
        for v1 in penultimosVerificadores:
            for v2 in unitarios:
    ...
    return [automatosVerDict, diagnosticoDict]


def BuscaExaustiva(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    minimumSubset = False
    # print("diagnose")
    bases = []
    ...
    for v in unitarios:
        verif.append(v)
    while len(diagnosticoDict) < limite:
        novosV = []
        for v1 in penultimosVerificadores:
            for v2 in unitarios:
                # print(unitarios)
                ...
                for base in bases:
                    eBase = []
                    for elemento in base.split("|"):
                        if elemento not in newV:
                            eBase.append(False)
                    if False not in eBase:
                        jaBase.append(True)
                        # print(base)
                        # print(newV)
                ...
                if v1 + "|" + v2 not in automatosVerDict.keys() and v2 + "|" + v1 not in automatosVerDict.keys() and v1 not in v2 and v2 not in v1 and len(
                        v2) == 4 and aux2 > aux1:
                    if True in jaBase:  # diagnosticoDict[v1]["diagnostico"] == ['Linguagem gerada é diagnosticável']:
                        diagnosticoDict[newV] = {"diagnostico": ['Linguagem gerada é diagnosticável'], "problemas": [],
                                                 "info": []}
                    else:
                        verificador1 = automatosVerDict[v1]
                        verificador2 = automatosVerDict[v2]
...
def BuscaExaustivaArvore(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    minimumSubset = False
    # print("diagnose")
    bases = []
    ...
    while len(diagnosticoDict) < limite:
        novosV = []
        for v1 in penultimosVerificadores:
            add_module_tree(v1, unitarios, bases, automatosVerDict, diagnosticoDict, verif, novosV, eventosRenomeados)
        penultimosVerificadores = novosV
        # print(len(automatosVerDict))
...
def add_module_tree(v1, unitarios, bases, automatosVerDict, diagnosticoDict, verif, novosV, eventosRenomeados):
    for v2 in unitarios:
        # print(unitarios)
        # print(penultimosVerificadores)
        # print(v1)
        ...
        if v1 + "|" + v2 not in automatosVerDict.keys() and v2 + "|" + v1 not in automatosVerDict.keys() and v1 not in v2 and v2 not in v1 and len(
            ...
            if True in jaBase:  # diagnosticoDict[v1]["diagnostico"] == ['Linguagem gerada é diagnosticável']:
                ...
                if diagnosticoDict[newV]["diagnostico"] == ['Linguagem gerada é diagnosticável']:
                    minimumSubset = True
                    bases.append(newV)
                add_module_tree(newV, unitarios, bases, automatosVerDict, diagnosticoDict, verif, novosV, eventosRenomeados)
...
def BuscaGfLinhaCompleta(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    minimumSubset = False
    # print("diagnose")
    bases = []
    ...
    while len(diagnosticoDict) < limite:
        ...
        for v1 in penultimosVerificadores:
            for v2 in unitarios:
                ...
                #             contBase = contBase + 1
                #             #umBase = False
                #         if contBase < len(b.split("|")):
                #             umBase = False
                #     jaBase.append(umBase)
                #
                # if True in jaBase:
                #     diagnosticoDict[newV] = {"diagnostico": ['Linguagem gerada é diagnosticável'], "problemas": [], "info": []}
...
def BuscaGfLinhaArvore(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    minimumSubset = False
    # print("diagnose")
    bases = []
    ...
    while len(diagnosticoDict) < limite:
        novosV = []
        for v1 in unitarios:  # penultimosVerificadores:
            add_module(v1, unitarios, bases, automatosVerDict, diagnosticoDict, auxGfLinhaDict, usaGfLinhaDict,
                       GfLinhaDict, verif, novosV, contGf, eventosRenomeados)
        penultimosVerificadores = novosV
        # print(len(automatosVerDict))
...
def add_module(v1, unitarios, bases, automatosVerDict, diagnosticoDict, auxGfLinhaDict, usaGfLinhaDict, GfLinhaDict,
               verif, novosV, contGf, eventosRenomeados):
    for v2 in unitarios:
        # print(unitarios)
        # print(penultimosVerificadores)
        # print(v1)
        ...
        if v1 + "|" + v2 not in automatosVerDict.keys() and v2 + "|" + v1 not in automatosVerDict.keys() and v1 not in v2 and v2 not in v1 and len(
            ...
            if True in jaBase:  # diagnosticoDict[v1]["diagnostico"] == ['Linguagem gerada é diagnosticável']:
                diagnosticoDict[newV] = {"diagnostico": ['Linguagem gerada é diagnosticável'], "problemas": [],
                                         "info": []}
                add_module(newV, unitarios, bases, automatosVerDict, diagnosticoDict, auxGfLinhaDict
                           , usaGfLinhaDict, GfLinhaDict, verif, novosV, contGf, eventosRenomeados)
            else:
                if v1 not in automatosVerDict.keys():
                    gflinha1 = auxGfLinhaDict[v1]
                    # proximas 4 linhas de codigo adicionadas para calcular o verificador quando não foi calculado anteriormente
                    ...
                    #GfLinhaDict["gf'" + v1] = gflinha1
                    # resp = AdicionaModuloEDiagnostica(automatosVerDict[v1[0:-5]], automatosVerDict[v1[-4:]])
                    # automatosVerDict[v1] = resp[2]
                    # printar = [resp[0], resp[1]]
                    # diagnosticoDict[v1] = {"diagnostico": resp[0], "problemas": resp[1], "info": resp[3]}
                    # print("diag")
                ...
                if ret[0] == ['Linguagem gerada é diagnosticável']:
                    if v1 not in automatosVerDict.keys():
                        # #adicionado 20220312 LR
                        max = len(v1)
                        aux = 0
                        while aux < max-8:
                            if v1[0:(aux+9)] not in automatosVerDict.keys():
                                print(v1[0:(aux+4)])
                                print(v1[(aux + 5): (aux + 9)])
                                resp = AdicionaModuloEDiagnostica(automatosVerDict[v1[0:(aux+4)]], automatosVerDict[v1[(aux+5):(aux+9)]], eventosRenomeados)
                                automatosVerDict[v1[0:(aux+9)]] = resp[2]
                                printar = [resp[0], resp[1]]
                                diagnosticoDict[v1[0:(aux+9)]] = {"diagnostico": resp[0], "problemas": resp[1], "info": resp[3]}
                            aux = aux+5

                        #print(v1)
                        #print(automatosVerDict.keys())
                        #print(automatosVerDict.values())
                        #fim adicao 20220312
                        #removidos em 20000312
                        #resp = AdicionaModuloEDiagnostica(automatosVerDict[v1[0:-5]], automatosVerDict[v1[-4:]])
                        #automatosVerDict[v1] = resp[2]
                        #printar = [resp[0], resp[1]]
                        #diagnosticoDict[v1] = {"diagnostico": resp[0], "problemas": resp[1], "info": resp[3]}
                        #fim remoção 20220312
                        # print("diag")
                        # print(diagnosticoDict[newV]["diagnostico"])
                    verificador1 = automatosVerDict[v1]
                    resp = AdicionaModuloEDiagnostica(verificador1, verificador2, eventosRenomeados)
                    ...
                    usaGfLinhaDict[v2] = gflinha1
                    #usaGfLinhaDict[newV] = gflinha1
                    # ADICIONAR A PARTE RECURSIVA AQUI?
                    add_module(newV, unitarios, bases, automatosVerDict, diagnosticoDict, auxGfLinhaDict
                               , usaGfLinhaDict, GfLinhaDict, verif, novosV, contGf, eventosRenomeados)
...
def BuscaTodos(automatosVerDict, autR, verUni, eventosRenomeados):
    ultimosVerificadores = []
    minimumSubset = False
    # print("diagnose")
    diagnosticoDict = {}
    ...
    for v in unitarios:
        verif.append(v)
    while len(automatosVerDict) < limite:
        novosV = []
        for v1 in penultimosVerificadores:
            for v2 in unitarios:
                # print(unitarios)
...
def BuscaPorLarguraGfLinha(dictAdjacencias={}, estado="", estados=[]):
    estadosAlcancados = []
    filaEstados = []
    situacaoEstado = {}
    situacaoEstado.clear()
...
def MontaGniCabral(a, cont, aut, Gn, eventosF):
    # cont = 1
    flagEeE = []
    flagEventos = []
    flagEstados = []
    # for a in aut:
    pre_a = automatoGn(a, eventosF)
    flagEeE = []
    flagEstados = []
    nome = "aut" + str(cont) + "n"
    # autn = limpaGni(automatoGni(a, Gn, eventosF))
    # print(nome)
    # for est in autn.estados:
    #  print(est)
    # for e in autn.EeE:
    #  print(e)
    # for est in a.estados:
    #  print(est)
    # for e in a.EeE:
    #   print(e)
    #REVISAR ESTA PARTE
    for ee in a.EeE:
        for een in Gn.EeE:
            eeSeparadoInput = ee[0].split('|')
            eeSeparadoOutput = ee[2].split('|')
            eenSeparadoInput = een[0].split('|')
            eenSeparadoOutput = een[2].split('|')
            contAux = (cont - 1) * 2
            if eeSeparadoInput[0] == eenSeparadoInput[contAux] and ee[1] == een[1] and eeSeparadoOutput[0] == eenSeparadoOutput[contAux] and ee[1] in pre_a.eventos:

            #if ee[0][0] == een[0][(cont - 1) * 4] and ee[1] == een[1] and ee[2][0] == een[2][(cont - 1) * 4] and ee[1] in pre_a.eventos:
            #if ee[0] == een[0][(cont - 1) * 4] and ee[1] == een[1] and ee[2] == een[2][(cont - 1) * 4] and ee[1] in pre_a.eventos:
            #if ee[0] == eenSeparadoInput[(2*cont - 1)] and ee[1] == een[1] and ee[2] == eenSeparadoOutput[(2*cont - 1)] and ee[1] in pre_a.eventos:
                if ee not in flagEeE:
                    flagEeE.append(ee)
                if ee[0] not in flagEstados:
                    flagEstados.append(ee[0])
                if ee[2] not in flagEstados:
                    flagEstados.append(ee[2])
    ...
    autn = a  # gni
    autn.EeE = flagEeE
    autn.eventos = pre_a.eventos
    autn.estados = flagEstados
    autAcessivel = ParteAcessivel(autn)
    return autAcessivel