...
def MontaGfLinha(verBase, caminho1, caminho2, estadoCFC1, estadoCFC2):
    ...
    if caminho2[0] == []:
        ...
        estCiclo = estAtual
        for ev in caminho1[2]:
            cont = cont + 1
            estProx = str(cont)
            estados.append(estProx)
            EeE.append([estAtual, ev, estProx])
            estAtual = estProx
        for ev in caminho2[2]:
            c = 1
...
def TesteGfLinha(gflinha, verNovo):
    teste = ComposicaoParalela(gflinha, verNovo)
    # print(teste.EeE)

    resp = VerificadorDiagnosticabilidade(teste, ['S1R1', 'S2R1', 'S1R2', 'S2R2', 'S1R3', 'S2R3', 'S1R4', 'S2R4'])
    printar = [resp[2], resp[1], teste, resp[3]]
    return printar


def AdicionaModuloEDiagnostica(verVelho, verNovo):
...
def BuscaGfLinha(automatosVerDict, autR, verUni, eventosRenomeados):
        ...
        for v1 in penultimosVerificadores:
            for v2 in unitarios:
                ...
                if v1 + "|" + v2 not in automatosVerDict.keys() and v2 + "|" + v1 not in automatosVerDict.keys() and v1 not in v2 and v2 not in v1 and len(
                    ...
                    ret = TesteGfLinha(gflinha1, verificador2)
                    GfLinhaDict["gf'" + v1 + "|" + v2 + "|" + str(contGf)] = ret[2]
                    # print("printar")
                    # print(printar)
                    if ret[0] == ['Linguagem gerada é diagnosticável']:
                        resp = AdicionaModuloEDiagnostica(verificador1, verificador2)
    ...
    # print(automatosVerDict)
    # print(diagnosticoDict)
    return [automatosVerDict, diagnosticoDict, GfLinhaDict]
